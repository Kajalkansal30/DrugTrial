...
async function checkPatientData() {
    try {
        ...
        try {
            ...
            console.log(`\n📊 Found ${results.length} patient screening records\n`);
            ...
            if (results.length > 0) {
                ...
                if (sample.details && sample.details.reasons) {
                    console.log('\nDetailed Breakdown:');
                    const reasons = sample.details.reasons;
                    console.log(`  Data Completeness: ${(reasons.data_completeness * 100).toFixed(0)}%`);
                    console.log(`  Inclusion Score: ${reasons.inclusion_score}`);
                    console.log(`  Exclusion Score: ${reasons.exclusion_score}`);
                    console.log(`  Hard Exclusions: ${reasons.hard_exclusions}`);
                }
            ...
            if (resultsError.response?.status === 404) {
                ...
                console.log('When you click "Re-run Screening", the data is saved to:');
